...
![image-20220111110323845](C:\Users\qinsz\AppData\Roaming\Typora\typora-user-images\image-20220111110323845.png)
...
以上两类调度算法在如Linux等操作系统中功能不同，因此需要分开实现，但在xv6操作系统中，由于主要设备仅为一个半虚拟磁盘virtio disk，因此两者功能较为重合，其中NOOP与FCFS基本一致，CFQ与CSCAN的实现也基本一致，因此我们接下来主要完成了NOOP，CFQ，SSTF与deadline算法
...
![img](file:///C:/Users/qinsz/AppData/Local/Temp/msohtmlclip1/01/clip_image012.png)
...
```c
int distance(struct req x,struct req y)//获取两个块的距离
{
...
void SSTF(struct Queue* qp)//SSTF算法实现，头尾指针改变在，在enqueue和dequeue中调用
{
...
* IOtest1.c利用父进程和子进程的方式对算法能否顺利通过调度进行了实现，建立串行读写的测试例，规定指定时间10s，测试共能完成多少次读写操作
* IOtest2.c实现随机读取，对算法进行进一步测试。
  使用预构建的随机数队列进行串行读写，在IOtest2中，输出这些调度请求传输的块的地址信息即可对NOOP和cfq算法进行比较，如果是xv6自带的NOOP算法，所有地址会以原序（即无序）输出，但是在cfq算法进行检定排序的每一个周期中，块的地址应当是有序的（代表请求也被排序）。
* IOtest3则采用了树状方法生成多进程，在叶子进程运行读写代码，并分别检验是否读写成功，属于并行读写。IOtest3记录每一个进程的读写时间，计算其总读写时间，对不同调度方法进行比较。
...
![image-20220111110246852](C:\Users\qinsz\AppData\Roaming\Typora\typora-user-images\image-20220111110246852.png)
...
| 9        | 409      | 366     | 349      | 374          |
| 10       | 364      | 343     | 366      | 377          |
| 平均值   | 366.7    | 367.8   | 362.6    | 378.2        |

* IOtest2（单位：ms）

| **组数** | **NOOP** | **CFQ** | **SSTF** | **Deadline** |
| -------- | -------- | ------- | -------- | ------------ |
| 1        | 2100     | 1700    | 1600     | 1500         |
| 2        | 1600     | 1900    | 1600     | 1600         |
| 3        | 1700     | 1800    | 1700     | 1600         |
| 4        | 1700     | 1700    | 1800     | 1700         |
| 5        | 1900     | 1800    | 1700     | 1800         |
| 6        | 1700     | 1800    | 1700     | 1800         |
| 7        | 1700     | 1700    | 1700     | 1700         |
| 8        | 1700     | 1700    | 1700     | 1800         |
| 9        | 1600     | 1700    | 1700     | 1800         |
| 10       | 1700     | 1800    | 1600     | 1700         |
| 平均值   | 1740     | 1760    | 1680     | 1700         |

* IOtest3（总周转时间+最大周转时间，单位：ms）

| **组数**   | **NOOP** | **CFQ** | **SSTF** | **Deadline** | **NOOP** | **CFQ** | **SSTF** | **Deadline** |
| ---------- | -------- | ------- | -------- | ------------ | -------- | ------- | -------- | ------------ |
| **1**      | 21200    | 19200   | 25400    | 24700        | 2800     | 2500    | 3100     | 3400         |
| **2**      | 26400    | 25600   | 22600    | 24500        | 3400     | 3200    | 2900     | 3300         |
| **3**      | 20800    | 23800   | 22200    | 21500        | 2700     | 3100    | 2900     | 2700         |
| **4**      | 22500    | 22500   | 25500    | 28300        | 2800     | 3100    | 3500     | 3500         |
| **5**      | 20700    | 24700   | 28100    | 22400        | 2900     | 3500    | 3500     | 2800         |
| **6**      | 22800    | 23300   | 27500    | 22100        | 2700     | 3200    | 3200     | 3200         |
| **7**      | 21900    | 22700   | 30700    | 20600        | 2600     | 3200    | 3600     | 3900         |
| **8**      | 20600    | 23600   | 22700    | 24000        | 2700     | 3200    | 3600     | 3200         |
| **9**      | 15100    | 22700   | 27500    | 19700        | 2400     | 3200    | 3400     | 2600         |
| **10**     | 24400    | 24500   | 27400    | 21700        | 3100     | 3200    | 3400     | 2900         |
| **平均值** | 21640    | 23260   | 25960    | 22950        | 2810     | 3140    | 3310     | 3150         |

3）结果分析

* 首先，在串行，并行两种调度请求的情况下，实现的各种算法都能够正常运作。

* 其次，通过对实验结果进行分析，我们可以发现各种算法在运行时间方面差距不大，甚至有CFQ等算法性能不如NOOP的情况。对此我们也提出了自己的思考：因为xv6采用的半虚拟磁盘机制，磁头扫动速度非常快，正常算法会以优化减少磁头移动距离等方式减小调度总时长，但是在该系统中这一优化并不能得到很好地体现。相反，为了实现调度算法我们认为构建了队列层，增加了一次调度过程，反而会让运算成本加大，从而导致处理时间的增长。所以系统原生的noop成为了效果最好的算法。

* 在我们实现的集中算法中，SSTF算法由于需要在每次运行时都需要比较一次和调度队列中所有请求的块距离，不论是最大周转时间还是处理所有请求所需的时间都要高于其他几种调度算法。而cfq和deadline算法由于采用了堆和树等排序方法对调度队列进行排序，调度的时间小于SSTF，符合这几种排序方法的性能。
